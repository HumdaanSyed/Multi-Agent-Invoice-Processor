...
## Current Phase
Phase 9A: Next.js scaffold and design system (docs/FRONTEND_PLAN.md) — the Verity frontend rebuild that replaces the original Phase 9. Scaffolded App Router + TypeScript + Tailwind v4 + shadcn/ui in **`web/`, not `frontend/`**: the Streamlit app still lives at `frontend/` and stays there until Phase 9E retires it, so the new app uses a temporary directory name to avoid colliding with it — Phase 9E renames `web/` to `frontend/` once Streamlit is deleted. Warm-paper color tokens and the three font roles (serif/sans/mono) are wired into `web/app/globals.css` with dark-mode support via `prefers-color-scheme`; `web/lib/config.ts` and `web/lib/api.ts`/`web/lib/types.ts` cover the product name and every backend endpoint including the SSE stream. Local dev note: the standard port 3000 was occupied by an unrelated process on this machine during development, so `.claude/launch.json`'s `web-frontend` config runs it on 3100 instead — `CORS_ALLOW_ORIGINS` in `.env` includes both. Phases 8.5 and 8.6 (streaming backend, export ledger) are merged. See docs/FRONTEND_PLAN.md for the full revised plan, phases 9B–9E.
