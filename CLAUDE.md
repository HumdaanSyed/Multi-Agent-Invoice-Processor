...
## Current Phase
Phase 9C: Human review (docs/FRONTEND_PLAN.md) — editable correction on top of Phase 9B's unified `/runs/[threadId]` view. `web/components/review-form.tsx` is the needs_review UI: fields a failed check implicates are amber and always editable (`CHECK_FIELDS` maps `check_id` → field names), everything else is editable behind an "Edit all fields" toggle; failures render in plain language computed from the invoice's own values (`explainFailure()`), not raw backend flag strings. `web/hooks/use-run.ts` gained `resume(corrections)`: opens a fresh SSE stream (the old one closed when the run interrupted) and calls `POST .../resume`, reusing the exact same field/check reveal machinery so a resume re-runs the identical live validation display. Fixed a real, high-impact bug found live-testing the resume flow: the hook closed its own SSE connection on the *first* `interrupted`/`run_end`-shaped event it saw, but `invoice_agent/events.py`'s channel history is cumulative across every resume, not per-connection — after a resume, a fresh subscriber's replay contains the *previous* validation cycle's stale terminal event before the real current one, and closing on it truncated the replay and caused a reconnect storm. Fixed by never closing inline on event type — only reactively, in response to the connection actually ending, using the last-seen event type to tell an expected server-initiated close from a real drop (see REVIEW.md). Verified live end-to-end: a broken invoice reaches the review screen, a partial fix (total only) re-fails and shows only the still-real duplicate-check failure with totals now read-only, and a full fix (total + invoice number) reaches `completed` and "Added to ledger". Phases 9A/9B are merged. See docs/FRONTEND_PLAN.md for Phase 9D (dashboard, detail, export bar) and 9E (retire Streamlit).
