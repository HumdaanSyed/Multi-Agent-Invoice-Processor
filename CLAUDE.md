...
## Current Phase
Phase 9B: Upload, live extraction, live validation (docs/FRONTEND_PLAN.md) — the flagship Verity screen. `web/app/page.tsx`'s upload zone reserves a thread_id, hands the File to the new `/runs/[threadId]` route via `web/lib/run-file-cache.ts`, and `web/hooks/use-run.ts` drives the live view: SSE for the animated field/stage/check events (client-side staggered ~200ms apart), the actual blocking `POST /invoices` call (or a plain `GET` when revisiting a run with no cached File) as the authoritative final state, and a polling fallback if the SSE connection drops. `web/components/{extraction,validation}-panel.tsx` render the two live panels; `needs_review`/`completed`/`skipped`/`failed` all render as states of the same route rather than separate pages — `/runs/[threadId]` is the one unified run view Phases 9C (editable review) and 9D (read-only detail + export bar) build on, not a new route each. Verified live end-to-end against the real backend/Anthropic/Supabase: field population, a genuine failed check with correct detail text, and a duplicate-flag review banner all confirmed in the browser; the all-pass "completed" branch is code-reviewed but not separately demoed since every sample PDF is already in this dev DB from earlier phases' testing. Fixed a real bug hit while wiring this up: `app/main.py` read `CORS_ALLOW_ORIGINS` before `.env` was loaded, so setting it only in `.env` (not the real shell env) silently had no effect. Phase 9A (scaffold, still at `web/` not `frontend/` — see prior note) is merged. See docs/FRONTEND_PLAN.md for phases 9C–9E.
